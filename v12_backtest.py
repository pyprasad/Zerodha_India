"""
v12_backtest.py — Market-Hours Time Fix + Paper Trading Mode
=============================================================
Changes from v11:
  1. TRADE TIMES: entry_date now stores 09:15 IST (NSE open), exit_date stores
     12:00 IST for stop/target intraday hits, 15:30 IST for signal/force-close exits.
     Previously all times were 00:00:00 (midnight) — incorrect for a market-hours system.
  2. PAPER TRADING: run_paper_trading() generates live signals for next session
     from latest market data. Saves state to reports/paper_trading_state.json.
...
def main():
    ...
    versions = [
        ...
        ("v12", f"Market-hours time fix (09:15/15:30 IST) + paper trading mode\n"
                f"        All 20 instruments, per-instrument optimal",
    ...
    fname = f"reports/v12_backtest_{ts}.json"
    all_data = {
        "version": "v12",
        "timestamp": ts,
        "discovery_grid": {
    ...
    console.print(Panel(
        ...
        f"v12 BEST: {ann:+.1f}% CAGR  |  {best_name}[/bold {'green' if ann >= 28 else 'yellow'}]\n\n"
        f"  Period       : {period}\n"
        ...
        title="[bold]v12 RESULT — MARKET HOURS FIX + PAPER TRADING READY[/bold]",
...
def run_paper_trading(best_per_inst=None, top2_per_inst=None, capital_file=None):
    ...
    Usage:
      ...
      from v12_backtest import run_paper_trading  # programmatic
    """
    ...
    if best_per_inst is None:
        ...
        if v12_files:
            ...
            console.print("[yellow]No v12 JSON found — running paper trading with BB reversion on all instruments[/yellow]")
