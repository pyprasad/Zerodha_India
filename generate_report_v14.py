...
v14 fix: trade times now reflect NSE market hours (09:15 entry, 15:30 exit).
...
VERSION_HISTORY = [
    ...
    ("v14",f"CRITICAL FIX: no trailing stop on entry bar · honest baseline",
...
<title>v14 Backtest — P&L Statement</title>
...
<!-- HEADER -->
<h1>v14 Algorithm — P&amp;L Statement <span class="v13-badge">VERSION 14</span></h1>
<p class="subtitle">
...
<div class="warning">
  <h4>⚠ Critical Bug Fixed in v14 — v11/v12/v13 Results Were Not Real</h4>
  <p>A user correctly identified that trade exits showed prices the market never reached (e.g. RELIANCE
  Apr-8 claimed exit ₹1,361.1, but the day's HIGH was only ₹1,350.6 — physically impossible).
  Root cause: <code>trail_stop(e21 − atr×0.3)</code> was called in mean-reversion exits (BB, WR).
  Since BB_LOW fires when price &lt; lower band &lt; e21, the trailing stop was always set
  <em>above</em> entry price — triggering fake profitable exits every bar.
  <strong>v13 partial fix</strong> blocked this on the entry day only; 4,515 profitable stops remained
  on subsequent bars (₹257.9M, 96.7% of v13 P&L was still artificial).
  <strong>v14 complete fix</strong>: trailing stops removed entirely from BB/WR mean-reversion exits.
  These trades now exit only via signal (BB_MID_EXIT/WR_EXIT) or the initial ATR stop.</p>
</div>
<div class="success">
  <h4>✅ v14 Honest Baseline: +{S['ann_ret']:.1f}% CAGR · Sharpe {S['sharpe']:.2f} · MaxDD {S['max_dd']:.1f}%</h4>
...
<div class="hero" id="hero">
  <div style="color:var(--text2);font-size:.9em;text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px">
    Algorithm v14 · Top-2 Strategies per Instrument · 2008–2026 · Annualised Return
  </div>
...
out_file = Path(f"reports/v14_PnL_Statement_{ts}.html")
