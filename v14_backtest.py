"""
v14_backtest.py — Complete Fix: Remove Trailing Stop from Mean-Reversion Exits
===============================================================================
BUG FOUND (v12 and earlier):
  ...
  Impact v12: 5,538 / 8,269 trades were fake. ₹346.8M of ₹350.8M reported
  P&L (98.9%) was fictional.

PARTIAL FIX (v13):
  Entry-bar guard added — skip trailing on the entry day only.
  BUT the bug persisted on day 2+: e21 remains above entry price for
  mean-reversion entries (BB_LOW fires when price < lower BB < e21).
  trail_stop(e21 − atr×0.3) still set stop above entry on subsequent bars.
  Impact v13: 4,515 profitable stops remaining, ₹257.9M fake P&L (96.7%).
  Proven: RELIANCE Apr-8 exit ₹1,361.1 — day HIGH was only ₹1,350.6.

COMPLETE FIX (v14):
  Remove pf.trail_stop() entirely from BB_ and WR_ exit handlers.
  Mean-reversion trades exit ONLY via:
    1. Signal exit (BB_MID_EXIT when %B crosses 0.5, WR_EXIT when WR crosses -50)
    2. Initial ATR stop set correctly below entry in enter()
    3. Target hit (2× risk-reward from enter())
  Trailing is conceptually wrong for mean-reversion: you enter because
  price is stretched, you exit when it snaps back — not by trailing.
...
def process_exit(pf, inst, date, ind, ii, vix_series):
    ...
    if tag.startswith("WR_") or tag.startswith("BB_"):
        ...
        if tag.startswith("WR_"):
            ...
            if d == -1 and pctp > 0.5 and pct <= 0.5:
                pf.force_close(inst, date, p, "BB_MID_EXIT"); return
        # v14 FIX: No trailing stop for mean-reversion entries.
        # Exit is handled by signal (BB_MID_EXIT/WR_EXIT) or initial ATR stop.
...
def main():
    ...
    versions = [
        ...
        ("v14", f"COMPLETE FIX: trailing stop removed from BB/WR mean-reversion exits\n"
                f"        96.7% of v13 P&L was still artificial — this is the honest result",
    ...
    fname = f"reports/v14_backtest_{ts}.json"
    all_data = {
        "version": "v14",
        "timestamp": ts,
        "discovery_grid": {
    ...
    console.print(Panel(
        ...
        f"v14 BEST: {ann:+.1f}% CAGR  |  {best_name}[/bold {'green' if ann >= 28 else 'yellow'}]\n\n"
        f"  Period       : {period}\n"
        ...
        title="[bold]v14 RESULT — HONEST BASELINE (MEAN-REVERSION TRAILING STOP BUG FIXED)[/bold]",
...
def run_paper_trading(best_per_inst=None, top2_per_inst=None, capital_file=None):
    ...
    Usage:
      ...
      from v14_backtest import run_paper_trading  # programmatic
    """
    ...
    if best_per_inst is None:
        v12_files = sorted(Path("reports").glob("v14_backtest_*.json"))
        if v12_files:
            ...
            console.print("[yellow]No v14 JSON found — running paper trading with BB reversion on all instruments[/yellow]")
